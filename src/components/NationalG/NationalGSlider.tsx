...
const NationalGSlider = () => {
  ...
  const getMovie = async () => {
    const data = await axios.get(
      "https://api.themoviedb.org/3/search/movie?query=National+Geographic&api_key=edd30aa38a66fd481aab68561f86ce5f"
    );
  ...
  const getImgElements = movieList.map((item) => {
    if (item.backdrop_path) {
      ...
      return (
        <img
          src={picture}
          className="min-w-full md:h-[650px] object-cover object-fill
         mr-5 rounded-md hover:border-[4px] border-gray-400"
        />
      );
    }
...
export default NationalGSlider;

// else {
//             return (
//               <img
//                 src={picture}
//                 className="min-w-full md:h-[650px] object-cover object-center
//          mr-5 rounded-md hover:border-[4px] border-gray-400"
//               />
//             );
//           }
